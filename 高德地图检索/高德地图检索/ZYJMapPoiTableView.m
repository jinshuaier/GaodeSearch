...
@implementation ZYJMapPoiTableView{
    
    UITableView    *_tableView;
    NSMutableArray *_searchPoiArray;//Poi搜索结果数组
    BOOL isFromMoreLoadRequest;//下拉更多请求数据的标记
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response{
    if (response.regeocode != nil) {
        ...
        ZYJModel *model = [[ZYJModel alloc]init];
        model.city = city;
        model.address = _selectedPoi.name;
        model.latitude = [NSString stringWithFormat:@"%f", _selectedPoi.location.latitude];
        model.longitude = [NSString stringWithFormat:@"%f", _selectedPoi.location.longitude];
        [self.delegate setCurrentCity:model];
